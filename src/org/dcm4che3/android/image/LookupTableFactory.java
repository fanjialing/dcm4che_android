...
public class LookupTableFactory {
    ...
    public boolean autoWindowing(Attributes img, Raster raster) {
        ...
        if (max == 0) {
            int[] min_max;
            min_max = calcMinMax(storedValue, raster,raster.getDataType());
    ...
    private int[] calcMinMax(StoredValue storedValue, Raster raster,int datatype) {
        ...
        int stride = raster.getWidth();
        for (int y = 0; y < h; y++)
            for (int i = y * stride, end = i + stride; i < end;i++) {
            	int v=0;
            	switch (datatype) {
				case Raster.TYPE_BYTE:
					v= raster.getByteData()[i];
					break;
				case Raster.TYPE_SHORT:
					v= raster.getShortData()[i];
					break;
